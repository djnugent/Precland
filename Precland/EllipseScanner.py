...
def Cart2Ind(offset,x):
    return np.round(offset+x).astype(int)
...
class EllipseScanner():
    ...
    def init_scan_ellipses(self, r_min, r_max, n_ellipses, n_scan_points):
        ...
        thetas_rad = np.linspace(0,2*np.pi,num=360,endpoint=False)

        # Convert from spherical to Cartesian
        x = np.outer(radii,np.cos(thetas_rad))
        y = np.outer(radii,np.sin(thetas_rad))
    ...
    def scan_img(self, img, h, w, origin, theta):
        ...
        for e in ellipses_affine:
            ellipses_img_ind.append(np.array([Cart2Ind(origin[0],e[0,:]),Cart2Ind(origin[1],e[1,:])]))
        ...
        return scan_result, ellipses_img_ind
